#!/usr/bin/env python3
"""
Training script for baseline LoRA model.
"""
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"Training LoRA on {args.dataset.upper()}")
    print("=" * 60)
    ...
    for epoch in range(training_config["num_epochs"]):
        ...
        if eval_results["loss"] < best_val_loss:
            ...
            lora_model.save_model(str(output_dir / "best_model"))
    ...
    # Load best model
    lora_model.load_model(str(output_dir / "best_model"))
    model = lora_model.model.to(args.device)
    ...
    # Save metrics
    metrics_tracker.save_metrics(f"{args.dataset}_lora_metrics.json")
