#!/usr/bin/env python3
"""
Training script for HiRA (High-Rank Adaptation) model.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train HiRA model")
    parser.add_argument(
        "--dataset",
        type=str,
        required=True,
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"Training HiRA on {args.dataset.upper()}")
    print("=" * 60)
    ...
    # Initialize model
    print("\nInitializing HiRA model...")
    task_type = dataset_config["task_type"]
    hira_model = HiRAModel(
        model_name=config["model"]["name"],
        task_type=task_type,
        num_labels=dataset_config.get("num_labels"),
        lora_config=hira_config,
    )

    model = hira_model.get_model()
    model.to(args.device)

    # Log model info
    model_info = hira_model.get_model_info()
    ...
    print(f"  Rank: {hira_config['r']} (High-rank)")
    print(f"  Alpha: {hira_config['lora_alpha']}")
    print(f"\nNote: This is a high-rank LoRA approximation of HiRA")
    print(f"      True HiRA uses Hadamard products for more expressiveness")
    ...
    for epoch in range(training_config["num_epochs"]):
        ...
        if eval_results["loss"] < best_val_loss:
            ...
            hira_model.save_model(str(output_dir / "best_model"))
    ...
    # Load best model
    hira_model.load_model(str(output_dir / "best_model"))
    model = hira_model.model.to(args.device)
    ...
    # Save metrics
    metrics_tracker.save_metrics(f"{args.dataset}_hira_metrics.json")
