#!/usr/bin/env python3
"""
Training script for Sparse LoRA model.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Sparse LoRA model")
    parser.add_argument(
        "--dataset",
        type=str,
        required=True,
...
def train_epoch(model, sparse_lora_model, dataloader, optimizer, scheduler, device, metrics_tracker, config):
    """Train for one epoch with sparsity."""
    model.train()
    total_loss = 0
    num_batches = 0
    ...
    for batch in progress_bar:
        ...
        if config.get("use_l1_regularization", False):
            l1_lambda = config.get("l1_lambda", 0.001)
            l1_loss = sparse_lora_model.compute_l1_loss()
...
def main():
    ...
    sparse_lora_config = config["sparse_lora"]
    ...
    print("\n" + "=" * 60)
    print(f"Training Sparse LoRA on {args.dataset.upper()}")
    print("=" * 60)
    ...
    # Initialize model
    print("\nInitializing Sparse LoRA model...")
    task_type = dataset_config["task_type"]
    sparse_lora_model = SparseLoRAModel(
        model_name=config["model"]["name"],
        task_type=task_type,
        num_labels=dataset_config.get("num_labels"),
        lora_config=sparse_lora_config,
    )

    model = sparse_lora_model.get_model()
    model.to(args.device)

    # Log model info
    model_info = sparse_lora_model.get_model_info()
    ...
    print(f"  Sparsity method: {sparse_lora_config['sparsity_method']}")
    print(f"  Target sparsity: {sparse_lora_config['sparsity_ratio'] * 100:.1f}%")
    ...
    prune_frequency = sparse_lora_config.get("prune_frequency", 1)
    ...
    for epoch in range(training_config["num_epochs"]):
        ...
        train_loss = train_epoch(
            model, sparse_lora_model, dataloaders["train"], optimizer, scheduler,
            args.device, metrics_tracker, sparse_lora_config
        )
        ...
        if eval_results["loss"] < best_val_loss:
            ...
            sparse_lora_model.save_model(str(output_dir / "best_model"))
    ...
    # Load best model
    sparse_lora_model.load_model(str(output_dir / "best_model"))
    model = sparse_lora_model.model.to(args.device)

    # Final sparsity statistics
    final_sparsity = sparse_lora_model.get_sparsity_stats()
    print(f"\nFinal Sparsity Statistics:")
    print(f"  Overall sparsity: {final_sparsity['overall_sparsity'] * 100:.2f}%")
    print(f"  Non-zero parameters: {final_sparsity['non_zero_params']:,}")
    print(f"  Total parameters: {final_sparsity['total_params']:,}")
    metrics_tracker.log_metrics(final_sparsity, prefix="final_sparsity")
    ...
    # Save metrics
    metrics_tracker.save_metrics(f"{args.dataset}_sparse_lora_metrics.json")
